...
static CGFloat const ACCESSORY_BUTTON_WIDTH = 20.0;
...
- (id)initWithToken:(CLToken *)token font:(nullable UIFont *)font {
    ...
    if (self) {
        ...
        [self.accessoryButton setAdjustsImageWhenHighlighted:NO];
...
- (CGSize)intrinsicContentSize {
    ...
    return CGSizeMake(labelIntrinsicSize.width+(2.0*PADDING_X) + ACCESSORY_BUTTON_WIDTH, labelIntrinsicSize.height+(2.0*PADDING_Y));
}
...
- (void)layoutSubviews {
    ...
    CGRect accessoryButtonFrame = CGRectMake(bounds.size.width - ACCESSORY_BUTTON_WIDTH,
                                             0.0,
                                             ACCESSORY_BUTTON_WIDTH,
                                             bounds.size.height);
    [self.accessoryButton setFrame:accessoryButtonFrame];
...
- (void)accessoryButtonTapped:(UIButton *)button {
    [self.delegate tokenViewDidReceiveTapOnAccessoryView:self];
}
